...
func (s *ExportServices) ExportSaleReturnsToPDF(branchID string, month string) ([]byte, error) {
	var saleReturns []models.SaleReturns
	...
	err := query.Order("return_date DESC").Find(&saleReturns).Error
	if err != nil {
		log.Printf("[ExportSaleReturnsToPDF] Query error: %v", err)
	...
	m.AddRows(
		row.New(9).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("RETUR PENJUALAN %s", month), props.Text{
					Size:  14,
					Align: "center",
	...
	headerRowContent := row.New(8).Add(
		...
		col.New(6).WithStyle(headerCell()).Add(text.New("SALE ID", headerTextProps())),
	...
	for _, sr := range saleReturns {
		grandTotal += sr.TotalReturn
	}

	for i, sr := range saleReturns {
		...
		m.AddRows(
			row.New(8).Add(
				col.New(2).WithStyle(cellStyle).Add(text.New(sr.ID, textProps)),
				col.New(6).WithStyle(cellStyle).Add(text.New(sr.SaleId, textProps)),
				col.New(2).WithStyle(cellStyle).Add(text.New(sr.ReturnDate.Format("02/01/2006"), textProps)),
				col.New(2).WithStyle(cellStyle).Add(text.New(formatRupiah(sr.TotalReturn), textProps)),
			),
		)
	...
	if err != nil {
		log.Printf("[ExportSaleReturnsToPDF] Generate error: %v", err)
