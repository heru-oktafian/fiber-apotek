...
func (s *ExportServices) ExportExpensesToPDF(branchID string, month string) ([]byte, error) {
	...
	if err != nil {
		log.Printf("[ExportExpensesToPDF] Query error: %v", err)
	...
	m.AddRows(
		row.New(9).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("PENGELUARAN %s", month), props.Text{
					Size:  14,
					Align: "center",
	...
	// Hitung total dari semua TotalExpense
	var grandTotal int
	...
	if err != nil {
		log.Printf("[ExportExpensesToPDF] Generate error: %v", err)
