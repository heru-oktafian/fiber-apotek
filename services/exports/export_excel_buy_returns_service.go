...
func (s *ExportServices) ExportBuyReturnsToExcel(branchID string, month string) ([]byte, error) {
	var returns []models.BuyReturns
	...
	f.SetCellValue(sheet, "A1", "RETUR PEMBELIAN "+month)
	...
	for i, r := range returns {
		...
		f.SetCellValue(sheet, fmt.Sprintf("B%d", row), r.PurchaseId)
	...
	if tableErr != nil {
		log.Printf("[ExportBuyReturnsToExcel] AddTable warning: %v", tableErr)
	}
