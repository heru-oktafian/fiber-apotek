...
func (s *ExportServices) ExportMemberCategoriesToExcel(branchID string) ([]byte, error) {
	...
	for i, cat := range categories {
		row := i + 4
		f.SetCellValue(sheet, fmt.Sprintf("A%d", row), fmt.Sprintf("%d", cat.ID))
	...
	if tableErr != nil {
		log.Printf("[ExportMemberCategoriesToExcel] AddTable warning: %v", tableErr)
	}
