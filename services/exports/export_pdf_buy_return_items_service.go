...
func (s *ExportServices) ExportBuyReturnItemsToPDF(branchID string, buyReturnID string) ([]byte, error) {
	var items []models.AllBuyReturnItems

	// Query data buy_return_items
	// Note: Perlu join products -> units untuk unit_name, karena di models.AllBuyReturnItems sepertinya memakai alias pro_id, pro_name
	...
	if err != nil {
		log.Printf("[ExportBuyReturnItemsToPDF] Query error: %v", err)
	...
	if err := s.db.Where("id = ? AND branch_id = ?", buyReturnID, branchID).First(&buyReturn).Error; err != nil {
		log.Printf("[ExportBuyReturnItemsToPDF] BuyReturn not found or mismatch branch: %v", err)
	...
	m.AddRows(
		row.New(9).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("ID RETUR PEMBELIAN : %s", buyReturn.ID), props.Text{
					Size:  14,
					Align: "center",
		...
		row.New(6).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("TANGGAL RETUR: %s", buyReturn.ReturnDate.Format("02/01/2006")), props.Text{
					Size:  10,
					Align: "center",
				}),
			),
		...
		row.New(6).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("ID PEMBELIAN : %s | METODE PEMBAYARAN : %s", buyReturn.PurchaseId, buyReturn.Payment), props.Text{
					Size:  10,
					Align: "center",
				}),
			),
	...
	headerRowContent := row.New(8).Add(
		...
		col.New(1).WithStyle(headerCell()).Add(text.New("QTY", headerTextProps())),
	...
	for i, item := range items {
		...
		m.AddRows(
			row.New(8).Add(
				col.New(5).WithStyle(cellStyle).Add(text.New(item.ProName, textProps)),
				col.New(2).WithStyle(cellStyle).Add(text.New(item.ExpiredDate.Format("02/01/2006"), textProps)),
				col.New(1).WithStyle(cellStyle).Add(text.New(fmt.Sprintf("%d %s", item.Qty, item.UnitName), textProps)),
	...
	m.AddRows(
		row.New(8).Add(
			...
			col.New(2).WithStyle(totalCellStyle).Add(text.New(formatRupiah(buyReturn.TotalReturn), totalValueProps)),
		),
	)
	...
	if err != nil {
		log.Printf("[ExportBuyReturnItemsToPDF] Generate error: %v", err)
