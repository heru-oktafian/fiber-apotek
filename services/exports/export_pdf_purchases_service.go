...
func (s *ExportServices) ExportPurchasesToPDF(branchID string, month string) ([]byte, error) {
	...
	if month != "" {
		...
		if err == nil {
			...
			query = query.Where("purchase_date >= ? AND purchase_date < ?", startDate, endDate)
		}
	}

	err := query.Order("purchase_date DESC").Find(&purchases).Error
	if err != nil {
		log.Printf("[ExportPurchasesToPDF] Query error: %v", err)
	...
	m.AddRows(
		row.New(9).Add(
			col.New(12).Add(
				text.New(fmt.Sprintf("PEMBELIAN %s", month), props.Text{
					Size:  14,
					Align: "center",
	...
	headerRowContent := row.New(8).Add(
		...
		col.New(6).WithStyle(headerCell()).Add(text.New("SUPPLIER", headerTextProps())),
	...
	// Hitung total dari semua TotalPurchase
	var grandTotal int
	...
	for i, p := range purchases {
		...
		m.AddRows(
			row.New(8).Add(
				col.New(2).WithStyle(cellStyle).Add(text.New(p.ID, textProps)),
				col.New(6).WithStyle(cellStyle).Add(text.New(p.SupplierId, textProps)),
				col.New(2).WithStyle(cellStyle).Add(text.New(p.PurchaseDate.Format("02/01/2006"), textProps)),
				col.New(2).WithStyle(cellStyle).Add(text.New(formatRupiah(p.TotalPurchase), textProps)),
			),
		)
	...
	if err != nil {
		log.Printf("[ExportPurchasesToPDF] Generate error: %v", err)
