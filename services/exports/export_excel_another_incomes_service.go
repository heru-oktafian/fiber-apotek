...
func (s *ExportServices) ExportAnotherIncomesToExcel(branchID string, month string) ([]byte, error) {
	var incomes []models.AnotherIncomes
	...
	err := query.Order("income_date DESC").Find(&incomes).Error
	if err != nil {
	...
	f.SetCellValue(sheet, "A1", "PENDAPATAN LAIN "+month)
	...
	for i, inc := range incomes {
		row := i + 4
		f.SetCellValue(sheet, fmt.Sprintf("A%d", row), inc.ID)
		f.SetCellValue(sheet, fmt.Sprintf("B%d", row), inc.Description)
		f.SetCellValue(sheet, fmt.Sprintf("C%d", row), inc.IncomeDate.Format("02/01/2006"))
		f.SetCellValue(sheet, fmt.Sprintf("D%d", row), string(inc.Payment))
		f.SetCellValue(sheet, fmt.Sprintf("E%d", row), formatRupiah(inc.TotalIncome))
		grandTotal += inc.TotalIncome
	...
	tableErr := f.AddTable(sheet, &excelize.Table{
		Range:             fmt.Sprintf("A3:E%d", len(incomes)+3),
		Name:              "AnotherIncomesTable",
	...
	if tableErr != nil {
		log.Printf("[ExportAnotherIncomesToExcel] AddTable warning: %v", tableErr)
	}
